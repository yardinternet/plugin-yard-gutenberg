...
/* harmony import */ var _helpers__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../helpers */ "./src/Blocks/helpers.js");
/**
...
const init = () => {
  ...
  collapses?.forEach(collapse => {
     ...
     */
    const accordion = new (accordion_js__WEBPACK_IMPORTED_MODULE_0___default())(collapse, {
      showMultiple,
      openOnInit,
      duration: 400
    });
    openCollapseItemByHash(accordion, collapseItems);
    window.addEventListener('hashchange', () => openCollapseItemByHash(accordion, collapseItems));
  });
};
...
const openCollapseItemByHash = (accordion, collapseItems) => {
  ...
  const collapseItemIndex = Array.from(collapseItems).findIndex(item => {
    const button = item.querySelector('.wp-block-yard-collapse-item__header-button');
    const text = extractHeaderText(button);
    const itemSlug = (0,_helpers__WEBPACK_IMPORTED_MODULE_1__.slugify)(text);
...
const extractHeaderText = headerButton => {
  ...
  const nodeSearchQuery = node => node.nodeType === Node.TEXT_NODE && node.nodeValue.trim() !== '';
