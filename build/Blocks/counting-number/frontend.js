...
!function() {
/*!************************************************!*\
  !*** ./src/Blocks/counting-number/frontend.js ***!
  \************************************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var countup_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! countup.js */ "./node_modules/countup.js/dist/countUp.min.js");

const init = () => {
  const countingNumbers = document.querySelectorAll('.wp-block-yard-counting-number__number');
  countingNumbers.forEach(number => {
    ...
    const observer = new IntersectionObserver(changes => handleCountUpObserver(changes, number), options);
...
const handleCountUpObserver = (changes, number) => {
  changes.forEach(change => {
    if (change.isIntersecting && change.boundingClientRect.top > 300) {
      ...
      const hasThousandsSeparator = parent.dataset.hasthousandsseparator === 'true';
      const animationDuration = Number(parent.dataset.animationduration);
      ...
      if (countUpInit.includes(',')) {
        options = {
          ...options,
          decimalPlaces: countUpInit.split(',').slice(-1)[0].length
        };
        countUpInit = countUpInit.replace(/\,/g, '.');
      }
      ...
      const countUp = new countup_js__WEBPACK_IMPORTED_MODULE_0__.CountUp(number, countUpInit, options);
      if (!countUp.error) {
        setTimeout(() => {
          countUp.start();
        }, 400);
