...
<<?php echo $heading; ?> <?php echo get_block_wrapper_attributes(['class' => 'wp-block-yard-tabs-item__heading']); ?>>
	<button id="tabs-item-button-<?php echo $id; ?>" class="wp-block-yard-tabs-item__button" role="tab" aria-controls="tabs-item-panel-<?php echo $id; ?>" aria-selected="false"><?php echo Render::icon($attributes), esc_html($attributes['headingText'] ?? ''); ?></button>
</<?php echo $heading; ?>>
<div class="<?php echo esc_attr($panelClass); ?>" id="tabs-item-panel-<?php echo $id; ?>" role="tabpanel" aria-hidden="true" aria-labelledby="tabs-item-button-<?php echo $id; ?>"><?php echo $content; ?></div>
